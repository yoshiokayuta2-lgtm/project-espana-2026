...
const packingGroups={
  ...
  '衣類':{icon:'○',fixed:{}},'洗面・ケア':{icon:'＋',fixed:{}},'機内・移動':{icon:'→',fixed:{}},'その他':{icon:'·',fixed:{shipcase:'10/16 スーツケース発送用：自宅住所・宅配準備'}}
};
...
const tripDays={'2026-10-08':{city:'Tokyo / Narita',title:'出発の日',img:'images/departure.png',items:[['Day','東京で過ごす','夜便に備えて余裕を持って移動'],['17:25','QR809 成田発','23:05 ドーハ着 · 隣席指定済み']]},'2026-10-09':{city:'Madrid',title:'マドリード到着',img:'images/madrid.png',items:[['01:10','QR147','07:35 マドリード着 · 隣席指定済み'],['Afternoon','Hotel Óperaへ','Ópera駅そば · 予約確定'],['Evening','最初の街歩き','無理のないペースで']]},'2026-10-10':{city:'Madrid',title:'芸術とフラメンコ',img:'images/corral.png',items:[['Morning','王宮・美術館','マドリードの文化を楽しむ'],['21:30','Corral de la Morería','Dinner + Show']]},'2026-10-11':{city:'Madrid → Barcelona',title:'AVEで移動',img:'images/madrid.png',items:[['Morning','Hotel Ópera Check-out','荷物を整えて駅へ'],['11:27','Madrid 発','Barcelonaへ列車移動'],['15:11','Barcelona 着','HCC Regenteへ']]},'2026-10-12':{city:'Barcelona',title:'バルセロナを歩く',img:'images/madrid.png',items:[['Morning','HCC Regenteから街歩き','Casa Batlló・La Pedreraも徒歩圏'],['Afternoon','ガウディ建築','街の空気を楽しむ']]},'2026-10-13':{city:'Barcelona',title:'サグラダと特別なディナー',img:'images/sagrada.png',items:[['09:30','HCC Regente → Sagrada Família','Basilica入場 · Quiet Hour'],['10:15','Nativity Tower','生誕のファサードの塔'],['19:00','Cinc Sentits','特別なディナー']]},'2026-10-14':{city:'Barcelona → Doha',title:'スペイン最終日',img:'images/selfie.png',items:[['Morning','HCC Regente Check-out','最後の街歩き・買い物'],['16:35','QR146','23:55 ドーハ着 · 隣席指定済み']]},'2026-10-15':{city:'Doha → Narita → Shin-Urayasu',title:'帰国と東京ベイ滞在',img:'images/flight.png',items:[['03:15','QR806','19:35 成田着 · 隣席指定済み'],['19:35','成田到着','入国・荷物受け取り'],['Night','オリエンタルホテル東京ベイ','新浦安駅直結・予約確定']]},'2026-10-16':{city:'Shin-Urayasu → Disney → Tokyo',title:'Disneyと旅の最終夜',img:'images/couple.png',items:[['Morning','Suitcases → Gifu','大型スーツケースを岐阜の自宅へ発送'],['Day','Tokyo Disney Resort','身軽になって一日楽しむ'],['Night','The Tokyo Station Hotel','Double Room · 朝食付き · BOOKED']]},'2026-10-17':{city:'Tokyo',title:'最後の東京と岐阜へ',img:'images/selfie.png',items:[['Morning','Hotel Breakfast','東京駅丸の内駅舎で迎える朝'],['Day','東京観光','浅草などを身軽に散策'],['Return','Tokyo → Gifu','東京駅から新幹線で帰宅']]}};
...
const memoryDayLabels={
  ...
  '2026-10-12':'Day 5 · Oct 12','2026-10-13':'Day 6 · Oct 13','2026-10-14':'Day 7 · Oct 14','2026-10-15':'Day 8 · Oct 15','2026-10-16':'Day 9 · Oct 16','2026-10-17':'Day 10 · Oct 17'
};
...
// Fun / Coin Flip v4.3.0
const coinFlipButton=document.getElementById('coinFlip');
const coinResult=document.getElementById('coinResult');
const coinTopic=document.getElementById('coinTopic');
const coinStorageKey='espana-coin-v1';
function renderCoinResult(data){
  if(!coinResult||!data)return;
  coinResult.innerHTML='';
  const small=document.createElement('small');small.textContent=data.topic||'RESULT';
  const strong=document.createElement('strong');strong.textContent=`${data.winner}!`;
  const span=document.createElement('span');span.textContent=data.winner==='Yuta'?'今日はYutaが決める番。':'今日はYukoが決める番。';
  coinResult.append(small,strong,span);
}
try{const saved=JSON.parse(localStorage.getItem(coinStorageKey)||'null');if(saved)renderCoinResult(saved)}catch{}
if(coinFlipButton){
  coinFlipButton.addEventListener('click',()=>{
    if(coinFlipButton.classList.contains('flipping'))return;
    coinFlipButton.classList.add('flipping');
    coinFlipButton.disabled=true;
    if(coinResult)coinResult.innerHTML='<small>FLIPPING</small><strong>Vamos...</strong><span>コインを投げています。</span>';
    const winner=Math.random()<.5?'Yuta':'Yuko';
    const topic=coinTopic?coinTopic.value:'Result';
    window.setTimeout(()=>{
      const result={winner,topic,time:Date.now()};
      renderCoinResult(result);
      localStorage.setItem(coinStorageKey,JSON.stringify(result));
      coinFlipButton.classList.remove('flipping');
